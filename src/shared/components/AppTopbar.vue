...
<template>
  <header class="sticky top-0 z-20 flex h-16 items-center justify-between gap-4 border-b border-surface-border bg-white/80 px-4 backdrop-blur lg:px-6">
    <div class="flex items-center gap-3">
      <button class="rounded-xl p-2 text-slate-500 hover:bg-primary-100 lg:hidden" :aria-label="t('common.menu')" @click="emit('toggleSidebar')">
    ...
    <div class="flex items-center gap-2">
      ...
      <div class="group relative">
        <button class="flex items-center gap-2 rounded-xl py-1 ps-1 pe-2 hover:bg-primary-100">
        ...
        <div class="absolute end-0 mt-1 hidden w-44 rounded-xl border border-surface-border bg-white py-1 shadow-elevated group-hover:block">
          <RouterLink to="/settings" class="block px-4 py-2 text-sm text-slate-600 hover:bg-primary-100">
            {{ t('common.settings') }}
          </RouterLink>
          <button class="block w-full px-4 py-2 text-start text-sm text-red-600 hover:bg-red-50" @click="signOut">
            {{ t('common.signOut') }}
          </button>
        </div>
      </div>
