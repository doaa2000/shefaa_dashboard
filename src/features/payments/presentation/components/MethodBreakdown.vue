...
<template>
  ...
  <div v-if="rows.length" class="rounded-2xl border border-surface-border bg-white p-5 shadow-card">
    <p class="text-sm font-medium text-slate-500">By payment method</p>
    <div class="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <div class="flex items-center justify-between">
          <p class="font-medium text-slate-800">{{ titleCase(row.method ?? '—') }}</p>
          <p class="text-xs text-slate-400">{{ row.count }}</p>
        </div>
        <p class="mt-1 text-sm text-emerald-600">{{ money(row.collected) }} collected</p>
